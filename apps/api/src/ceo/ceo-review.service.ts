...
export class CeoReviewService {
  ...
  async snapshot(companyId: string) {
    ...
    const [lastRun, runsThisWeek, leadsByStatus, sentThisWeek, positiveThisWeek, projects, paid, account, survival, directives, answers, ventures, categories] =
      await Promise.all([
  ...
  async runReview(companyId: string, ceoEmployeeId: string, simTime: Date) {
    ...
    try {
      const res = await this.gateway.generate({ systemMessage: REVIEW_SYSTEM, prompt: JSON.stringify(snap), requireStructuredOutput: true, temperature: 0.2 });
      review = normalizeReview(res.structuredOutput);
    } catch (e) {
  ...
  async answerChairman(companyId: string, question: string) {
    ...
    const res = await this.gateway.generate({
      systemMessage: `You are the CEO of a small AI-run web/automation agency in India, answering your Chairman directly.
...
If the data can't answer the question, say so and say what you'd need.`,
      prompt: JSON.stringify({ chairmanQuestion: q, business: snap, lastReview: recent && { at: recent.reviewedAt, topRisk: recent.topRisk, topOpportunity: recent.topOpportunity, actions: recent.decisionsProposed } }),
      temperature: 0.3,
    });
    return { question: q, answer: res.text.trim(), answeredAt: new Date() };
  }
