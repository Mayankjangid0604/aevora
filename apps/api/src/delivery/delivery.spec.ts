...
describe('Phase 44 budget tiers', () => {
  const { getBudgetTier, ModelTier } = require('@aevora/model-gateway');
  it('maps quoted paise to model tiers at the boundaries', () => {
    expect(getBudgetTier(500000)).toBe(ModelTier.LOCAL_BASIC);
    expect(getBudgetTier(500001)).toBe(ModelTier.SONNET);
    expect(getBudgetTier(1000000)).toBe(ModelTier.SONNET);
    expect(getBudgetTier(2000000)).toBe(ModelTier.OPUS);
    expect(getBudgetTier(2000001)).toBe(ModelTier.FABLE);
  });
});
